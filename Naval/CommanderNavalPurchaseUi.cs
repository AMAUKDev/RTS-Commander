    ...
    private void DrawWindow(int windowId)
    {
        if (CommanderUiTheme.DrawHelpButton(windowRect.width, ref helpVisible))
        {
            CommanderUiTheme.DrawHelpOverlay(
                ...
                "A naval dock unlocks all of this. Build one from the BUILD window on dry land at the water's edge, within the naval build radius of a base you hold; each of its three levels opens a heavier class of hull, and the enemy commander is on exactly the same ladder. Then purchase a ship with faction funds and select a water rally point on the fullscreen map. The ship enters from a clear Sea Lane near the friendly map edge and uses normal Basegame ship navigation.");
        }
        if (GUI.Button(
        ...
        int dockLevel = CommanderNavalPurchaseService.DockLevel;
        GUI.Label(
        ...
        GUI.Label(
            ...
            dockLevel <= 0
                ? "NO NAVAL DOCK — BUILD ONE ON THE SHORE FROM THE BUILD WINDOW"
                : $"NAVAL DOCK LVL {dockLevel}/{CommanderEconomyService.MaxLevel}   |   "
                    + CommanderNavalPurchaseService.GetLevelUnlockLabel(dockLevel).ToUpperInvariant(),
        ...
        bool listEnabled = GUI.enabled;
        for (int i = 0; i < service.ShipDefinitions.Count; i++)
        {
            ShipDefinition definition = service.ShipDefinitions[i];
            bool unlocked = CommanderNavalPurchaseService.IsUnlocked(definition, dockLevel);
            GUIStyle style = ReferenceEquals(definition, selected)
                ? CommanderUiTheme.SelectedButton
                : CommanderUiTheme.Button;
            // Locked hulls stay on the list rather than disappearing, so the dock ladder reads as
            // something to build toward instead of a list that mysteriously grows.
            string label = unlocked
                ? service.GetDefinitionLabel(definition)
                : $"{service.GetDefinitionLabel(definition)}   [DOCK LVL "
                    + $"{CommanderNavalPurchaseService.GetRequiredDockLevel(definition)}]";
            GUI.enabled = listEnabled && unlocked;
            if (GUI.Button(new Rect(4f, 4f + i * 58f, viewRect.width - 8f, 50f), label, style))
            {
        ...
        GUI.enabled = oldEnabled
            && selected != null
            && CommanderNavalPurchaseService.IsUnlocked(selected, dockLevel);
        if (GUI.Button(
